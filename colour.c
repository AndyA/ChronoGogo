...
void colour_f_rgb2yuv(const colour_floats *in, colour_floats *out) {
  out->c[cY] = 16 + SCF(in->c[cR], 65.738) + SCF(in->c[cG], 129.057) +
               SCF(in->c[cB], 25.064);

  out->c[cCb] = 128 - SCF(in->c[cR], 37.945) - SCF(in->c[cG], 74.494) +
                SCF(in->c[cB], 112.439);

  out->c[cCr] = 128 + SCF(in->c[cR], 112.439) - SCF(in->c[cG], 94.154) -
                SCF(in->c[cB], 18.285);
...
void colour_f_yuv2rgb(const colour_floats *in, colour_floats *out) {
  out->c[cR] = SCF(in->c[cY], 298.082) + SCF(in->c[cCr], 408.583) - 222.921;

  out->c[cG] = SCF(in->c[cY], 298.082) - SCF(in->c[cCb], 100.291) -
               SCF(in->c[cCr], 208.120) + 135.576;

  out->c[cB] = SCF(in->c[cY], 298.082) + SCF(in->c[cCb], 516.412) - 276.836;
